...
struct KeyValues
{
    ...
    bool isEmpty() const
    {
        return keys.size() != values.size() || keys.isEmpty();
    }
    int size() const
    {
        return qMin(values.size(), keys.size());
    }
};
...
class QCP_LIB_DECL QCustomPlot : public QWidget
{
  ...
  Q_PROPERTY(bool tracerAccuracy READ tracerAccuracy WRITE setTracerAccuracy)
  /// \endcond
public:
  /*!
  ...
  */
  ...
  int tracerAccuracy() const {return mTracerAccuracy; }
  bool tracerEnable() const {return mTracerEnable; }
  QCPItemText *tracerLabel() { return mTracerLabel; }
  QCPItemTracer *tracer() { return mTracer; }

  // cursor support
  QList<QCPAbstractCursor *> mCursors;
  QCPAbstractCursor *mContextCursor = nullptr; // 菜单选中的Cursor
  QCPAbstractCursor *mActiveCursor = nullptr;
  void addCursor(QCPAbstractCursor *cursor);
  void removeCursor(QCPAbstractCursor *cursor);
  void showPlotContextMenu(const QPoint &globalPos, const QPoint &localPos);
  void showCursorContextMenu(const QPoint &globalPos);
  ...
  void setTracerAccuracy(int tracerAccuracy) {mTracerAccuracy = tracerAccuracy;}
  void setTracerEnable(bool enable) {mTracerEnable = enable;}
  ...
  QCPCurve *curve(int index) const;
  QCPGraph *graph() const;
  QCPCurve *curve() const;
  QCPGraph *addGraph(QCPAxis *keyAxis=nullptr, QCPAxis *valueAxis=nullptr);
  QCPCurve *addCurve();
  QCPItemText *addItemText();
  QCPItemTracer *addItemTracer();
  QCPItemStraightLine *addItemStraightLine();
  bool removeGraph(QCPGraph *graph);
  bool removeGraph(int index);
  int clearGraphs();
  int graphCount() const;
  int curveCount() const { return mCurves.size(); }
...
protected:
  ...
  void updateTracerLabelAlignment();
  ...
  bool setupOpenGl();
  void freeOpenGl();
  void setTracerAndTooltip(const QString &name, QCPAxis *xAxis, QCPAxis *yAxis, double x, double y, double yReal=0);
...
public:
  ...
  */
  ...
  void setValueGainOffset(double gain, double offset) { mValueGain = gain; mValueOffset = offset; }
  ...
  // *** PYSIDE6 FIX ***
  void clearData() {mDataContainer.data()->clear();}
  bool replaceValue(int index, double newValue);
...
/* =========================================
 * QCPAbstractCursor
 * Base class for cursor-like helpers
 * ========================================= */

class QCP_LIB_DECL QCPAbstractCursor : public QObject
{
  Q_OBJECT
public:
  ...
  explicit QCPAbstractCursor(QCustomPlot *parentPlot);
  virtual ~QCPAbstractCursor();
  ...
  // graph binding
  void setBindGraph(QCPGraph *graph);
  QCPGraph *bindGraph() const { return mBindGraph; }

  // interaction
  virtual DragTarget selectTest(const QPointF &pos) const = 0;
  virtual void startDrag(const QPointF &pos);
  virtual void dragTo(const QPointF &pos);
  virtual void endDrag();

  // position update (plot coord x)
...
signals:
  ...
  void bindGraphChanged(QCPGraph *graph);
  void positionChanged();

protected:
  QCustomPlot *mParentPlot = nullptr;
  QCPGraph *mBindGraph = nullptr;
...
/* =========================================
 * QCPVerticalCursorGroup
 * ========================================= */
class QCP_LIB_DECL QCPVerticalCursorGroup : public QCPAbstractCursor
{
  Q_OBJECT
public:

  explicit QCPVerticalCursorGroup(QCustomPlot *parentPlot);
  virtual ~QCPVerticalCursorGroup();

  void setX(double x1, double x2);
  double xLeft() const { return mX1; }
  double xRight() const { return mX2; }

  void setColor(const QColor &c);
  void setDeltaPosition(double y);

  // QCPAbstractCursor interface
  DragTarget selectTest(const QPointF &pos) const override;
...
protected:
  ...
  QCPItemStraightLine *mLeftLine = nullptr;
  QCPItemStraightLine *mRightLine = nullptr;
  QCPItemTracer *mTracerLeft = nullptr;
  QCPItemTracer *mTracerRight = nullptr;
  QCPItemLine *mDeltaLine = nullptr;
  QCPItemText *mTextLeft = nullptr;
  QCPItemText *mTextRight = nullptr;
  QCPItemText *mTextDelta = nullptr;

  double mDeltaY = 0.0;
  double mX1,mY1 = 0.0;
  double mX2,mY2 = 0.0;

  mutable DragTarget mDragTarget = dtNone;
};
